  ...
  provideLinter: () => {
    return {
      ...
      lint: (activeEditor) => {
        ...
        const regex_deprec_warning = /\[DEPRECATION WARNING\]:\s(.*)/;
        ...
        return helpers.exec(atom.config.get('linter-ansible-syntax.ansibleExecutablePath'), args, {stream: 'stderr'}).then(output => {
          ...
          output.split('\r?\n').forEach(function (line) {
            ...
            const matches_deprec_warning = regex_deprec_warning.exec(line);
            ...
            if (matches_deprec_warning != null) {
              toReturn.push({
                type: 'Warning',
                range: 1,
                text: matches_deprec_warning[1],
                filePath: file
              });
            }
          });
