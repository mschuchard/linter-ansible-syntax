...
export default {
  ...
  activate() {
    ...
    helpers.exec(atom.config.get('linter-ansible-syntax.ansibleExecutablePath'), ['--help']).then(output => {
      if (!(/--become/.exec(output))) {
    ...
    return {
      ...
      lint: (activeEditor) => {
        ...
        return helpers.exec(atom.config.get('linter-ansible-syntax.ansibleExecutablePath'), args, {cwd: require('path').dirname(file), stream: 'stderr', allowEmptyStderr: true}).then(output => {
          ...
          // capture range and error
          const matches_range = regex_range.exec(output);
          const matches_error = regex_error.exec(output);

          // grab the included file if the error is elsewhere; also, check if the output is guessing at which file has the error
          var the_file = file;
          ...
          if (matches_error != null) {
            toReturn.push({
              ...
              location: {
                file: the_file,
                position: [[Number.parseInt(matches_range[1]) - 1, Number.parseInt(matches_range[2]) - 1], [Number.parseInt(matches_range[1]) - 1, Number.parseInt(matches_range[2])]],
              },
            });
          }
          // check for warnings
